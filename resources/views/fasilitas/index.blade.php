<x-app-layout>
    ...
    <div class="py-10 bg-gradient-to-b from-cyan-50 via-sky-50 to-white min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @if (session('success'))
                <div class="mb-6 bg-green-50 border-l-4 border-green-500 text-green-700 px-4 py-3 rounded-lg shadow-sm font-semibold flex items-center gap-2" role="alert">
            ...
            <div class="relative overflow-hidden rounded-3xl shadow-xl mb-8 text-white bg-gradient-to-br from-cyan-500 via-sky-600 to-blue-800">
                ...
                <div class="relative z-10 p-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div>
                        ...
                        <h3 class="text-2xl md:text-3xl font-extrabold mb-1">Daftar Fasilitas & Alat Wisata</h3>
                        <p class="text-cyan-50/90 text-sm">Kelola gazebo, paddle board, dan alat sewa lainnya di sini.</p>
                    </div>

                    <a href="{{ route('fasilitas.create') }}" class="inline-flex items-center gap-2 px-5 py-3 bg-white text-blue-700 font-bold rounded-xl hover:bg-cyan-50 transition shadow-lg text-sm whitespace-nowrap">
                            ...
                            @forelse ($fasilitas as $index => $item)
                                <tr class="hover:bg-cyan-50/50 transition">
                                    ...
                                    <td class="py-3 px-6 text-left">
                                        @if($item->gambar)
                                            <img src="{{ asset($item->gambar) }}" alt="{{ $item->nama_fasilitas }}" class="w-16 h-16 object-cover rounded-xl border border-cyan-100 shadow-sm">
                                        @else
                                            <div class="w-16 h-16 rounded-xl bg-gradient-to-br from-cyan-100 to-sky-200 flex items-center justify-center text-cyan-500">
                                                <svg class="w-6 h-6 opacity-60" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14M4 20h16a1 1 0 001-1V5a1 1 0 00-1-1H4a1 1 0 00-1 1v14a1 1 0 001 1z"/></svg>
                                            </div>
                                        @endif
                                    </td>

                                    <td class="py-3 px-6 text-left font-bold text-gray-800">{{ $item->nama_fasilitas }}</td>
                                    <td class="py-3 px-6 text-center">
                                        <span class="bg-cyan-100 text-cyan-700 py-1 px-3 rounded-full text-xs font-bold">{{ $item->kategori }}</span>
                                    </td>
                                    <td class="py-3 px-6 text-right font-extrabold text-blue-700">
                                        Rp {{ number_format($item->harga, 0, ',', '.') }}
                                        <span class="text-xs font-normal text-gray-400 block">/ {{ $item->satuan }}</span>
                                    </td>
                                    <td class="py-3 px-6 text-center">
                                        <div class="flex items-center justify-center gap-2">

                                            <a href="{{ route('fasilitas.edit', $item->id) }}" class="w-9 h-9 flex items-center justify-center bg-amber-100 text-amber-600 rounded-lg hover:bg-amber-400 hover:text-white transition shadow-sm" title="Ubah Data">
                                ...
                                <tr>
                                    <td colspan="6" class="py-16 px-6 text-center">
                                        <div class="w-16 h-16 mx-auto mb-4 rounded-full bg-cyan-50 flex items-center justify-center">
                                            <svg class="w-8 h-8 text-cyan-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5"/></svg>
                                        </div>
                                        <p class="text-gray-700 font-bold">Data fasilitas belum ada.</p>
                                        <p class="text-gray-400 text-sm mt-1">Silakan tambahkan data baru.</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
